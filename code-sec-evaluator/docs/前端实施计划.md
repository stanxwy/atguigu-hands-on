...
| 里程碑 | 内容 | 产出 | 验收要点 |
| --- | --- | --- | --- |
| M1 工程初始化 | 创建 `frontend/` 脚手架、配置文件、主题、类型、枚举、Axios client | 可 `npm run dev` 启动、`tsc --noEmit` 通过 | 基础工程与代理可用 |
| M2 认证与布局 | authStore、登录/初始化页、AppLayout、ProtectedRoute/AdminRoute | 登录可跳转、登出可用、布局响应式 | AC-1 |
| M3 项目模块 | 项目列表/创建/详情、启动/停止/删除、分页与状态筛选 | 项目 CRUD 闭环 | AC-2、AC-6 |
