...
| 后端（FastAPI） | 源码部署：`venv` + `pip install` + Uvicorn | systemd（自定义 unit） | 仅 `127.0.0.1:8000` |
...
# 4. 虚拟环境 + 安装依赖
sudo -u evaluator bash -lc "cd /data/code-sec-evaluator/app/backend \
  && python3.11 -m venv /data/code-sec-evaluator/venv \
  && source /data/code-sec-evaluator/venv/bin/activate \
...
ExecStart=/data/code-sec-evaluator/venv/bin/uvicorn app.main:app \
...
| 升级类型 | 步骤 | 回滚 |
| --- | --- | --- |
| 代码升级 | `git fetch --tags && git checkout <tag>` → `pip install -r requirements.txt` → `systemctl restart code-sec-evaluator` | `git checkout <prev-tag>` + 重启 |
...
pip install -r requirements.txt           # 3. 依赖
...
deploy-production:
  ...
  script:
    ...
    - ssh deploy@prod "cd /data/code-sec-evaluator/app/backend && git checkout $CI_COMMIT_TAG && pip install -r requirements.txt && alembic upgrade head && systemctl restart code-sec-evaluator"
```
...
| 依赖清单 | `requirements.txt`/`package-lock.json` 锁定精确版本（SPEC §5.1） |
